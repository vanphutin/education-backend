...
import { errorBody, type MovieView } from './contracts.js';
import { json, requestIdOf } from './http.js';

export interface GatewayOptions { catalogBaseUrl: string; timeoutMs: number }
...
export function createGatewayServer(options: GatewayOptions): Server {
  return createServer(async (request, response) => {
    ...
    try {
      ...
      if (!upstream.ok) throw new Error(`catalog_status_${upstream.status}`);
      const payload = await upstream.json() as { data: MovieView[] };
